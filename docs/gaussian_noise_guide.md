...
高斯噪声在SLAM系统中用于模拟真实传感器数据中的不确定性和误差，帮助测试算法在存在噪声环境下的鲁棒性。本项目采用**双阶段、双种子**架构，分别在子地图配准前和图优化前注入噪声。
...
| 你的需求 | 配置方案 |
|---------|---------|
| **完全可复现的实验** | `add_gaussian_noise: true`<br>`noise_seed_dr: 20`<br>`noise_seed_submap: 21` |
| **随机压力测试** | `add_gaussian_noise: true`<br>删除或注释掉两个种子 |
| **无噪声基线** | `add_gaussian_noise: false` |
| **在线/离线对比** | 两次运行使用**完全相同**的种子值 |

### 核心概念速览

**四个关键参数**：
1. `add_gaussian_noise` - **总开关**，控制是否注入任何噪声
2. `noise_seed_dr` - **DR边种子**，控制图优化约束的噪声
3. `noise_seed_submap` - **子地图种子**，控制GICP配准的噪声
...
- 离线模式：先所有子图加噪，再所有DR边加噪
- **问题**：单种子会导致两种模式从同一序列取不同值
- **解决**：双种子独立控制，保证DR边噪声一致
...
❌ **错误2**：使用旧参数 `noise_seed`
```yaml
noise_seed: 20  # 已废弃，不再使用
```

❌ **错误3**：只设置一个种子
```yaml
noise_seed_dr: 20
# 缺少 noise_seed_submap，仍无法完全复现
```
...
在 `config.yaml` 文件中，有三个与高斯噪声相关的参数：

```yaml
add_gaussian_noise: true    # 是否在模拟数据中添加高斯噪声
noise_seed_dr: 20          # DR边噪声种子（影响图优化约束）
noise_seed_submap: 21      # 子地图噪声种子（影响GICP配准）
```

### add_gaussian_noise
- **类型**: `boolean`
- **默认值**: `true`
- **功能**: 控制是否在SLAM图的DR边和子地图上添加高斯噪声
- **影响**: 当设置为 `true` 时，系统会在DR边和子地图上分别添加噪声

### noise_seed_dr
- **类型**: `integer`（可选）
- **默认值**: `20`（示例值）
- **功能**: 设置DR边噪声生成器的种子，控制图优化约束的噪声可复现性
- **使用方法**:
  - 设置具体数值：每次运行DR边产生相同的噪声模式
  - 注释掉或删除：DR边噪声保持完全随机

### noise_seed_submap
- **类型**: `integer`（可选）
- **默认值**: `21`（示例值）
- **功能**: 设置子地图噪声生成器的种子，控制GICP配准输入的噪声可复现性
- **使用方法**:
  - 设置具体数值：每次运行子地图产生相同的噪声模式
  - 注释掉或删除：子地图噪声保持完全随机
...
系统支持在线和离线两种SLAM模式，它们对噪声注入的时机要求不同：

- **离线模式**：先完成全部图构建（包含子地图噪声），再统一对所有DR边加噪，最后优化
  - 随机数消耗顺序：`[submap0噪声] → [submap1噪声] → ... → [DR0噪声] → [DR1噪声] → ...`
  
- **在线模式**：每处理一个子地图，立即对新增DR边加噪并增量优化
  - 随机数消耗顺序：`[DR0噪声] → [submap1噪声] → [DR1噪声] → [submap2噪声] → ...`

如果使用单一随机种子，两种模式会从同一序列中以不同顺序消耗随机数，导致即使种子相同，DR边拿到的也是完全不同的噪声值，无法实现在线/离线结果对比。

**双种子方案的优势**：

1. **解耦噪声源**：DR边噪声和子地图噪声使用独立生成器，互不干扰
...
```
配置参数层
├── add_gaussian_noise (总开关)
│   ├── true  → 启用噪声注入
│   └── false → 跳过所有噪声注入逻辑
│
├── noise_seed_dr (DR边随机源)
│   ├── 设置值 → DR边使用固定种子
│   └── 未设置 → DR边使用随机种子
│
├── noise_seed_submap (子地图随机源)
│   ├── 设置值 → 子地图使用固定种子
│   └── 未设置 → 子地图使用随机种子
│
└── online_opt_enable (影响DR边注入时机)
    ├── true  → 在线模式：DR边逐边加噪 + 增量优化
    └── false → 离线模式：DR边批量加噪 + 全局优化
```
...
| 参数 | 作用范围 | 影响 | 默认值 |
|------|---------|------|--------|
| `add_gaussian_noise` | **全局噪声总开关** | 控制是否执行任何噪声注入 | `true` |
| `noise_seed_dr` | **DR边噪声生成器** | 控制图优化约束的随机性 | `20` |
| `noise_seed_submap` | **子地图噪声生成器** | 控制GICP配准输入的随机性 | `21` |
| `online_opt_enable` | **SLAM模式选择** | 决定DR边噪声注入时机与优化策略 | `false` |
...
启动程序
    │
    ├─→ 检查 add_gaussian_noise
    │      ├─ false → 跳过所有噪声注入，进入纯SLAM流程
    │      └─ true  → 继续 ↓
    │
    ├─→ 初始化随机生成器
    │      ├─ DR边生成器: 读取 noise_seed_dr (有值用固定种子，无值用随机)
    │      └─ 子地图生成器: 读取 noise_seed_submap (有值用固定种子，无值用随机)
    │
    ├─→ 检查 online_opt_enable
    │      ├─ false → 离线批处理模式 ↓
    │      │     ├─ 遍历子地图
    │      │     │   ├─ 有重叠 → 从 submap生成器 取噪声 → 加到子地图 (第一阶段)
    │      │     │   ├─ 创建DR边 (不加噪，留到后面)
    │      │     │   └─ GICP配准、创建回环
    │      │     ├─ 图构建完成 → 从 DR生成器 批量取噪声 → 加到所有DR边 (第二阶段)
    │      │     └─ 全局Ceres优化
    │      │
    │      └─ true → 在线增量模式 ↓
    │            ├─ 遍历子地图
    │            │   ├─ 有重叠 → 从 submap生成器 取噪声 → 加到子地图 (第一阶段)
    │            │   ├─ 创建DR边 → 立即从 DR生成器 取噪声 → 加到当前DR边 (第二阶段)
    │            │   ├─ GICP配准、创建回环
    │            │   └─ 满足频率 → 增量Ceres优化
    │            └─ 输出在线误差日志
    │
...
```
离线模式 (online_opt_enable=false):
─────────────────────────────────────────────────────────
子地图0 → submap生成器 → [噪声值A]
子地图1 (有重叠) → submap生成器 → [噪声值B]
子地图2 (有重叠) → submap生成器 → [噪声值C]
...
子地图N (有重叠) → submap生成器 → [噪声值X]
...
在线模式 (online_opt_enable=true):
─────────────────────────────────────────────────────────
子地图0 → (不加噪，初始无重叠)
  创建DR边0 → DR生成器 → [噪声值1]

子地图1 (有重叠) → submap生成器 → [噪声值B]
  GICP → 创建DR边1 → DR生成器 → [噪声值2]
  → 增量优化(可选)

子地图2 (有重叠) → submap生成器 → [噪声值C]
  GICP → 创建DR边2 → DR生成器 → [噪声值3]
  → 增量优化(可选)
...
如果使用单一种子`noise_seed`，两种模式会从**同一个随机序列**中以**不同顺序**取值：

```
假设随机序列: [r1, r2, r3, r4, r5, r6, ...]
...
在线模式消耗:
  DR边0 取 r1  ← DR边拿到 r1 (与离线不同!)
  submap1 取 r2
  DR边1 取 r3  ← DR边拿到 r3 (与离线不同!)
  submap2 取 r4
```
...
在线模式:
  DR边0 取 d1  ← DR边拿到 d1 (与离线一致!)
  submap1 取 s1
  DR边1 取 d2  ← DR边拿到 d2 (与离线一致!)
  submap2 取 s2
```

**结论**：双种子保证了无论在线/离线，"第i条DR边"和"第j个子地图"拿到的噪声值完全相同，实现真正的可比较性。
...
- **噪声分布**: 正态分布，均值为0
- **种子控制**: 两个阶段使用**独立的随机数生成器**，分别受`noise_seed_dr`和`noise_seed_submap`控制
- **被禁用的噪声**: 平移噪声和roll/pitch旋转噪声在代码中被注释掉
- **在线/离线差异**: 
  - **离线模式**：DR边噪声在图构建完成后批量注入（保证随机序列一致性）
  - **在线模式**：DR边噪声在创建时逐边注入（模拟实时误差累积）
...
```yaml
add_gaussian_noise: true
noise_seed_dr: 20          # DR边噪声种子
noise_seed_submap: 21      # 子地图噪声种子
```
...
```yaml
add_gaussian_noise: true
noise_seed_dr: 20          # DR边使用种子20
noise_seed_submap: 21      # 子地图使用种子21
```

每次运行程序时，将产生完全相同的噪声模式（无论在线/离线模式）。
...
```yaml
# 场景1：DR边可复现，子地图随机（测试图优化稳定性）
add_gaussian_noise: true
noise_seed_dr: 20          # 固定DR噪声
# noise_seed_submap: 21    # 注释掉，子地图随机

# 场景2：子地图可复现，DR边随机（测试GICP鲁棒性）
add_gaussian_noise: true
# noise_seed_dr: 20        # 注释掉，DR边随机
noise_seed_submap: 21      # 固定子地图噪声
```

### 配置组合示例与适用场景

#### 场景1：完全可复现实验（推荐用于发论文/对比算法）
```yaml
add_gaussian_noise: true
noise_seed_dr: 20
noise_seed_submap: 21
online_opt_enable: false   # 或 true，根据需要
```
**效果**：每次运行完全一致，便于版本对比和结果复现

#### 场景2：测试算法鲁棒性（随机压力测试）
```yaml
add_gaussian_noise: true
# noise_seed_dr: 20        # 注释掉两个种子
# noise_seed_submap: 21
online_opt_enable: false   # 或 true
```
**效果**：每次运行产生不同噪声，测试算法对随机扰动的容忍度
...
noise_seed_dr: 20
noise_seed_submap: 21
online_opt_enable: false

# 第二次运行（在线模式）
add_gaussian_noise: true
noise_seed_dr: 20          # 保持相同
noise_seed_submap: 21      # 保持相同
online_opt_enable: true
```
**效果**：两次运行DR边噪声完全一致，可对比在线增量 vs 离线批处理的优化效果
...
online_opt_enable: false   # 或 true
```
**效果**：纯SLAM算法性能，无任何噪声干扰，作为对比基线

#### 场景5：仅测试GICP鲁棒性（DR边无噪声）
```yaml
add_gaussian_noise: true
# noise_seed_dr: 20        # 不设置DR种子（或设为0禁用）
noise_seed_submap: 21
online_opt_enable: false
```
**注意**：当前实现中，`add_gaussian_noise=true` 会同时启用两阶段噪声。若需单独测试，需在代码中临时注释 `addNoiseToGraph` 调用。

### 参数配置注意事项

#### ⚠️ 常见错误配置

**错误1：期望在线/离线结果一致，但未使用双种子**
```yaml
# ❌ 错误配置
add_gaussian_noise: true
# 缺少 noise_seed_dr 和 noise_seed_submap
online_opt_enable: true/false
```
**问题**：每次运行随机种子不同，无法对比
...
add_gaussian_noise: true
noise_seed_dr: 20
# 缺少 noise_seed_submap
```
**问题**：DR边可复现，但子地图噪声随机，仍无法完全复现

#### ✅ 正确配置检查清单

- [ ] 确认 `add_gaussian_noise` 设置符合测试需求
- [ ] 若需可复现，**必须同时设置** `noise_seed_dr` 和 `noise_seed_submap`
- [ ] 对比实验时，确保两次运行使用**完全相同**的种子值
- [ ] 检查终端输出，确认"实际种子"与配置一致
- [ ] 在线/离线对比时，确保 `noise_seed_dr` 和 `noise_seed_submap` 完全相同
...
```yaml
noise_seed_dr: 1847206849      # 使用之前运行时显示的DR边种子值
noise_seed_submap: 2938475610  # 使用之前运行时显示的子地图种子值
```

### 在线模式与离线模式的差异

系统支持两种运行模式，噪声注入策略略有不同：

**离线批处理模式** (`online_opt_enable: false`)：
- DR边噪声：在图构建完成后统一批量注入
- 子地图噪声：在GICP配准前逐个注入
- 随机数消耗顺序：先所有子地图噪声，后所有DR边噪声
- 优化：一次性全局优化

**在线增量模式** (`online_opt_enable: true`)：
- DR边噪声：每创建一条DR边立即注入（模拟实时误差累积）
- 子地图噪声：在GICP配准前逐个注入
- 随机数消耗顺序：DR边噪声与子地图噪声交错
- 优化：按频率触发增量优化

**为什么采用双种子？**

由于两种模式的随机数消耗顺序不同，如果使用单一种子，同样的种子值会导致DR边拿到完全不同的噪声，无法实现在线/离线结果对比。使用双种子后：
- DR边总是从`noise_seed_dr`生成器取值，与模式无关
- 子地图总是从`noise_seed_submap`生成器取值，与模式无关
- 保证相同种子 + 相同数据 = 可比较的结果
...
遇到噪声相关问题？
    ...
    │      │     └─ false → 没有噪声，无需种子
    │      ├─ 检查: noise_seed_dr 和 noise_seed_submap 是否都设置？
    │      │     └─ 缺少任一个 → 该部分使用随机种子
    │      ├─ 检查: 终端输出的"实际种子"是否与配置一致？
    ...
    │      ├─ 检查: add_gaussian_noise 是否为 true？
    │      │     └─ false → 噪声注入被全局禁用
    │      ├─ 检查: 终端是否输出"添加高斯噪声"相关信息？
    │      │     └─ 没有 → 可能在GICP前无重叠，子地图噪声未触发
    │      └─ 检查: poses_corrupted.txt 与 poses_original.txt 是否有差异？
    │            └─ 无差异 → 噪声未生效或噪声过小
    │
    └─→ 问题4: 种子值无效/被忽略
           ├─ 检查: config.yaml 中是否使用了旧参数 noise_seed？
           │     └─ 是 → 已废弃，需改为 noise_seed_dr 和 noise_seed_submap
           ├─ 检查: 种子值是否为整数？
           │     └─ 字符串/浮点数会导致解析失败
           └─ 检查: YAML 缩进是否正确？
                 └─ 缩进错误会导致参数读取失败
...
- 确认函数签名更新（`runOffline` 现需4个生成器参数）

### 运行时问题
- 验证 `config.yaml` 语法正确（使用 YAML 验证工具）
- 确认 `noise_seed_dr` 和 `noise_seed_submap` 为整数类型
- 检查程序输出中的种子设置确认信息：
  ```
  DR边噪声种子已设置为: 20 (用户指定)
  子地图噪声种子已设置为: 21 (用户指定)
...
- 对比文件验证：
  - `poses_original.txt` - 无噪声的原始轨迹
  - `poses_corrupted.txt` - 加噪后优化前的轨迹
  - `poses_optimized.txt` - 优化后的轨迹

### 种子相关问题
- **问题**: 设置了种子但每次运行结果仍不同
  - **解决**: 检查终端输出确认两个种子都被正确设置，确保没有其他随机源干扰
- **问题**: 想复现之前的运行但忘记记录种子
  - **解决**: 下次运行时注意记录终端输出的两个种子值
- **问题**: 自动生成的种子值很大，不容易记忆
  - **解决**: 可以在配置文件中设置简单的种子值，如`noise_seed_dr: 20`, `noise_seed_submap: 21`
- **问题**: 在线模式和离线模式结果差异很大
  - **解决**: 
    1. 确认使用了相同的`noise_seed_dr`和`noise_seed_submap`
    2. 检查是否同时启用了`add_gaussian_noise`
    3. 双种子机制保证DR边噪声一致，但优化策略不同会导致最终结果有差异
- **问题**: 只想测试某一阶段的噪声效果
  - **解决**: 可以只设置一个种子，另一个注释掉使用随机值，或在代码中临时关闭某阶段的噪声注入
